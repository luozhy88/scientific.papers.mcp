...
export class BioRxivDriver extends BaseDriver {
    textExtractor;
    bioRxivApiBase = "https://api.biorxiv.org";
    medRxivApiBase = "https://api.medrxiv.org";
    ...
    async fetchLatest(category, count) {
        ...
        try {
            ...
            // Build API URL
            const apiBase = server === "medrxiv" ? this.medRxivApiBase : this.bioRxivApiBase;
            ...
            const response = await axios.get(url, {
                ...
                timeout: 15000,
                headers: {
                    "User-Agent": "SciHarvester-MCP/0.1.27 (mailto:contact@sciharvestermcp.org); bioRxiv-client",
                },
            });
            ...
            if (subject !== "all") {
                papers = papers.filter((paper) => paper.category &&
            ...
            const paperMetadata = await Promise.all(validPapers.map((paper) => this.convertPaperToMetadata(paper, false)));
            ...
            for (const server of servers) {
                try {
                    ...
                    if (response.data &&
                        ...
                        const paperMetadata = await this.convertPaperToMetadata(paper, true);
    ...
    async searchPapers(query, field, count, sortBy) {
        ...
        throw new Error("Search functionality is not supported by bioRxiv/medRxiv API. " +
    ...
    async convertPaperToMetadata(paper, includeText = false) {
        ...
        // Construct URLs
        const server = paper.server || (paper.doi.includes("medrxiv") ? "medrxiv" : "biorxiv");
        ...
        if (includeText) {
            ...
            try {
                ...
                if (this.checkRateLimit()) {
                    try {
                        const extractionResult = await this.textExtractor.extractText(paperUrl);
                        ...
                        if (paper.abstract && paper.abstract.length > 0) {
                            ...
                            logInfo("Using abstract as text content for bioRxiv/medRxiv paper", {
                                id: paper.doi,
                                server,
                                textLength: paperMetadata.text.length,
                            });
                            ...
                            logWarn("All text extraction strategies failed for bioRxiv/medRxiv paper", {
                                id: paper.doi,
                                server,
                                htmlError: htmlError instanceof Error ? htmlError.message : htmlError,
                            });
                        }
                    }
                }
...
//# sourceMappingURL=biorxiv-driver.js.map