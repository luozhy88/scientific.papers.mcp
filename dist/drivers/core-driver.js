...
export class CoreDriver extends BaseDriver {
    textExtractor;
    apiBase = "https://api.core.ac.uk/v3";
    apiKey;
    ...
    constructor(rateLimiter) {
        ...
        if (!this.apiKey) {
            logWarn("CORE API key not found in environment variables. Using public rate limits.");
    ...
    async fetchLatest(category, count) {
        ...
        try {
            ...
            const headers = {
                "User-Agent": "SciHarvester-MCP/0.1.27 (mailto:contact@sciharvestermcp.org); CORE-client",
            };
            if (this.apiKey) {
                headers["Authorization"] = `Bearer ${this.apiKey}`;
            }
            const response = await axios.post(`${this.apiBase}/search/works`, {
                q: searchQuery,
            ...
            const validResults = results.filter((result) => result.title && result.id);
            const papers = await Promise.all(validResults.map((result) => this.convertPaperToMetadata(result, false)));
    ...
    async fetchContent(id) {
        ...
        try {
            ...
            const response = await axios.get(`${this.apiBase}/works/${id}`, {
                timeout: 15000,
                headers,
            });
            if (!response.data) {
     ...
     */
    async convertPaperToMetadata(paper, includeText = false) {
        // Extract authors
        ...
        if (paper.downloadUrl) {
            ...
            const pdfLink = paper.links.find((link) => link.type === "download" && link.url.includes(".pdf"));
            if (pdfLink) {
                pdf_url = pdfLink.url;
            }
        }
        else if (paper.sourceFulltextUrls &&
        ...
        if (includeText) {
            ...
            try {
                ...
                if (pdf_url && this.checkRateLimit()) {
                    try {
                        const extractionResult = await this.textExtractor.extractText(pdf_url);
                ...
                if (paperMetadata.text.length === 0 &&
                    ...
                    for (const sourceUrl of paper.sourceFulltextUrls) {
                        try {
                            const extractionResult = await this.textExtractor.extractText(sourceUrl);
                            if (extractionResult.extractionSuccess) {
                                paperMetadata.text = extractionResult.text;
                                textTruncated = extractionResult.truncated;
    ...
    buildSearchQuery(category) {
        const categoryMapping = {
            ...
            chemistry: 'subjects:("chemistry" OR "chemical" OR "biochemistry" OR "molecular")',
            biology: 'subjects:("biology" OR "biological" OR "genetics" OR "molecular biology")',
            medicine: 'subjects:("medicine" OR "medical" OR "health" OR "clinical")',
            engineering: 'subjects:("engineering" OR "technology" OR "mechanical" OR "electrical")',
            social_sciences: 'subjects:("social science" OR "sociology" OR "anthropology" OR "social")',
            economics: 'subjects:("economics" OR "economic" OR "business" OR "finance")',
            psychology: 'subjects:("psychology" OR "psychological" OR "cognitive" OR "behavioral")',
            education: 'subjects:("education" OR "educational" OR "learning" OR "pedagogy")',
            linguistics: 'subjects:("linguistics" OR "language" OR "linguistic" OR "phonetics")',
            philosophy: 'subjects:("philosophy" OR "philosophical" OR "ethics" OR "logic")',
            history: 'subjects:("history" OR "historical" OR "cultural studies" OR "heritage")',
            geography: 'subjects:("geography" OR "geographic" OR "environmental" OR "spatial")',
            law: 'subjects:("law" OR "legal" OR "jurisprudence" OR "justice")',
            arts: 'subjects:("arts" OR "literature" OR "cultural" OR "humanities")',
            agriculture: 'subjects:("agriculture" OR "agricultural" OR "farming" OR "food security")',
            environmental_science: 'subjects:("environmental" OR "sustainability" OR "ecology" OR "climate")',
            political_science: 'subjects:("political science" OR "politics" OR "governance" OR "policy")',
        };
        // Use predefined mapping or fallback to general search
        return categoryMapping[category.toLowerCase()] || `subjects:"${category}"`;
    }
}
//# sourceMappingURL=core-driver.js.map