...
export class OpenAlexDriver extends BaseDriver {
    textExtractor;
    doiResolver;
    politePoolEmail = "contact@sciharvestermcp.org";
     ...
     */
    getRequestParams(additionalParams = {}) {
        return {
    ...
    async convertWorkToPaper(work, includeText = false) {
        ...
        const pdf_url = fullTextResult.source === "pdf"
            ? fullTextResult.url
        ...
        if (includeText) {
            ...
            try {
                ...
                if (fullTextResult.url &&
                    ...
                    this.checkRateLimit()) {
                    const extractionResult = await this.textExtractor.extractText(fullTextResult.url);
    ...
    async searchPapers(query, field, count, sortBy) {
        ...
        try {
            ...
            // Process works in parallel for better performance (metadata only)
            const paperPromises = response.data.results.map((work) => this.convertWorkToPaper(work, false));
            const papers = await Promise.all(paperPromises);
            ...
            if (axios.isAxiosError(error)) {
                if (error.response?.status === 403) {
                    throw new Error(`OpenAlex API access forbidden - invalid search query: ${query}`);
                }
                if (error.response?.status === 429) {
                    throw new Error("Rate limited by OpenAlex API");
                }
