...
export class PMCDriver extends BaseDriver {
    ...
    pmcBase = "https://www.ncbi.nlm.nih.gov/pmc";
    ...
    async fetchLatest(category, count) {
        ...
        try {
            ...
            const papers = await Promise.all(validSummaries.map((summary) => this.convertSummaryToPaper(summary, false)));
     ...
     */
    async convertSummaryToPaper(summary, includeText = false) {
        // Extract authors
    ...
    async searchPapers(query, field, count, sortBy) {
        ...
        throw new Error("Advanced search functionality is not fully supported by PMC E-utilities API. " +
    ...
    buildSearchQuery(category) {
        const categoryMapping = {
            ...
            bioinformatics: '"bioinformatics"[MeSH Terms] OR "computational biology"[All Fields]',
            public_health: '"public health"[MeSH Terms] OR "epidemiology"[All Fields]',
        };
        // Use predefined mapping or fallback to general search
        return (categoryMapping[category.toLowerCase()] || `"${category}"[All Fields]`);
    }
    /**
...
//# sourceMappingURL=pmc-driver.js.map