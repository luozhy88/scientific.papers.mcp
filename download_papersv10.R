#!/usr/bin/env Rscript
# =============================================================================
# 学术文献搜索与 PDF 下载工具（多源 PDF 增强版 2.6 - 摘要清洗优化版）
# 更新说明: 
# v2.6: 严格限制摘要长度，防止全文误入 CSV；优化摘要提取正则逻辑
...
CONFIG <- list(
  ...
  MAX_ABSTRACT_LEN = 4000  # 摘要最大长度限制，防止全文写入 CSV
)
...
limit_text_length <- function(x, max_len = CONFIG$MAX_ABSTRACT_LEN) {
  ...
  if (nchar(x) > max_len) {
    # 如果超过最大长度，截断并添加标记
    return(paste0(substr(x, 1, max_len), "... [Truncated/Content too long]"))
  }
  x
}
...
extract_abstract_from_text <- function(text) {
  ...
  # 正则 1: 匹配 Abstract: ... 到下一个大写标题或数字列表前
  ...
  # 如果没有明确找到 "Abstract" 关键字，返回空字符串，而不是返回全文
  ""
}
...
extract_abstract_from_content_res <- function(res, src) {
  ...
  # 最后尝试从 text 字段提取，但必须通过正则验证
  # 如果 text 字段是全文，extract_abstract_from_text 会尝试寻找 "Abstract" 关键字
  # 如果找不到关键字，它现在会返回空字符串，而不是返回整个全文
...
main <- function() {
  ...
  cat("\n╔═══════════════════════════════════════════════════════════╗\n")
  cat("║   学术文献搜索与 PDF 下载工具（多源 PDF 增强版 2.6）     ║\n")
  cat("╚═══════════════════════════════════════════════════════════╝\n\n")
  ...
  for (src in CONFIG$SOURCES) {
    ...
    if ("abstract" %in% names(df)) {
      df$abstract <- sapply(seq_len(nrow(df)), function(i) {
        ...
        limit_text_length(abst) # 初始获取时也限制一下
      })
    } else {
      df$abstract <- ""
    }
  ...
  # ── CSV 写入前清洗：同时处理 1. 换行符/双引号 2. 文本超长截断 ──
