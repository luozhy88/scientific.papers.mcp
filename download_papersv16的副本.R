...
#      - 新增多关键词相关性过滤：AND 查询时自动过滤无关论文
#        （如 "insomnia AND AI" 只返回同时包含两个词的论文）
#      - EuropePMC 查询自动规范化 AND/OR/NOT 大小写
#      - 重写说明文档，与代码实现保持一致
#
# v3.5: 高档期刊优先版
...
# 将用户输入的查询标准化（小写 and/or/not → 大写 AND/OR/NOT）
normalize_query <- function(q) {
  q <- trimws(q)
  q <- gsub("\\band\\b", "AND", q, ignore.case = TRUE, perl = TRUE)
  q <- gsub("\\bor\\b",  "OR",  q, ignore.case = TRUE, perl = TRUE)
  q <- gsub("\\bnot\\b", "NOT", q, ignore.case = TRUE, perl = TRUE)
  q
}

# 从查询中提取 AND 必须出现的词组列表
# 返回: list，每个元素是一组 OR 可选词（字符向量）
# 例: "insomnia AND AI"                   → list(c("insomnia"), c("AI"))
# 例: "insomnia AND (AI OR deep learning)"→ list(c("insomnia"), c("AI","deep learning"))
# 例: "diabetes"（单词）                  → list(c("diabetes"))
extract_required_terms <- function(q) {
  q_norm <- normalize_query(q)

  if (!grepl("\\bAND\\b", q_norm)) {
    # 纯 OR 查询：不强制所有词，返回空列表（全部通过）
    if (grepl("\\bOR\\b", q_norm)) return(list())
    # 单词/短语：整体作为必须匹配的词组
    term <- trimws(gsub("\\bNOT\\b.*", "", q_norm))
    term <- trimws(gsub('[\"()]', "", term))
    if (nzchar(term)) return(list(c(term)))
    return(list())
  }

  # 有 AND：每个 AND 子句均必须满足
  parts    <- strsplit(q_norm, "\\bAND\\b")[[1]]
  required <- list()
  for (part in parts) {
    part <- trimws(gsub('[\"()]', "", part))
    if (grepl("^NOT\\b", part, perl = TRUE)) next   # 跳过 NOT 子句
    if (!nzchar(part)) next
    if (grepl("\\bOR\\b", part)) {
      # OR 子句：任一词出现即满足
      alts <- trimws(strsplit(part, "\\bOR\\b")[[1]])
      alts <- alts[nzchar(alts)]
      if (length(alts) > 0) required <- c(required, list(alts))
    } else {
      required <- c(required, list(c(part)))
    }
  }
  required
}
...
# 检查论文是否与查询相关
# 对 AND 查询：每个必须词组至少有一个词出现在标题或摘要中
is_paper_relevant <- function(required_terms, title, abstract = "") {
  if (length(required_terms) == 0) return(TRUE)
...
search_europepmc_r <- function(q, n, days) {
  ...
  q_norm    <- normalize_query(q)   # 标准化 AND/OR/NOT 大小写
  query_str <- sprintf("%s AND (FIRST_PDATE:[%s TO %s] OR PDATE:[%s TO %s])",
                       q_norm, date_from, date_to, date_from, date_to)
